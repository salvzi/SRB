...
export default {
  ...
  methods: {
      ...
      this.$alert("您即将前往汇付宝投标", "投标", {
        ...
        callback: (action) => {
          if (action === "confirm") {
            ...
            this.$axios
              .$post("/api/core/lendItem/commitInvest", this.invest)
              .then((response) => {
    ...
    getUserAccount() {
      // 账户余额
      this.$axios
        .$get("/api/core/userAccount/getUserAccount")
        .then((response) => {
    ...
    showUserType() {
      ...
      if (userInfoStr) {
        let userJSON = JSON.parse(userInfoStr);
        this.userType = userJSON.userType;
      }
    },
  },
};
