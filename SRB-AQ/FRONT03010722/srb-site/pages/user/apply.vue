...
export default {
  ...
  mounted() {
    this.$axios
      .$get("/api/core/borrowInfo/getBorrowInfoStatusById")
      .then((response) => {
        this.borrowInfoStatus = response.data.borrowInfoStatus;
        if (this.borrowInfoStatus === 0) {
          this.active = 0;
          this.getDictSelectedByDictCode(); // 初始化下拉列表
          this.getBorrowAmount(); // 初始化借款额度
        }
  ...
  methods: {
    ...
    getBorrowAmount() {
      this.$axios
        .$get("/api/core/integralGrade/getBorrowAmount")
        .then((response) => {
    ...
    getDictSelectedByDictCode() {
      this.$axios
        .$get("/api/core/dict/getDictSelectedByDictCode/returnMethod")
        .then((response) => {
          this.returnMethodList = response.data.dictList;
        });

      this.$axios
        .$get("/api/core/dict/getDictSelectedByDictCode/moneyUse")
        .then((response) => {
