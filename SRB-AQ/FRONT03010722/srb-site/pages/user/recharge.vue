...
export default {
  ...
  methods: {
    commitCharge() {
      this.$alert("您即将跳转第三方汇付宝平台充值", "汇付宝充值", {
        dangerouslyUseHTMLString: true,
        confirmButtonText: "立即前往",
        callback:(action)=>{
            if(action==="confirm"){
                this.$axios.$post("/api/core/userAccount/commitCharge/"+this.chargeAmt).then(response=>{
                    this.$message.error("跳转汇付宝")
                    document.write(response.data.form);
                })
            }else{
                this.$message.error("取消充值")
            }
        }
      });
    },
